...
- Be **backend-pluggable**: a local test-CA (port of `sign.py`) for CI, with room for real signing
  services (Azure Key Vault / PKCS#11 / ESRP) without touching the orchestration.
...
2. For each, call the backend's `preflight()` (§6) — a **cheap, side-effect-free** capability probe:
   ...
   - **remote reachable** — for `azure-key-vault`/`pkcs11`/`esrp`, required credentials/modules are
...
- **`keypair`** (BYO) — load a PEM key + cert and sign with `sbsign`. The "we already have a Secure
  Boot cert" case.
- **`azure-key-vault` / `pkcs11` / `esrp`** (future) — the key can't be handed to `sbsign`, so tailor
...
```mermaid
graph LR
  S1["S1: pipeline + lean stack<br/>(rcgen + sbsign-in-container)"] --> S2["S2: remote backends<br/>(Key Vault / PKCS#11 / ESRP)"]
...
- **S2 — remote backends.** `azure-key-vault` / `pkcs11` / `esrp` behind the same `Signer` trait. As
