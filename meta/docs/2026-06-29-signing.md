...
> **Update (2026-07-22):** this exact `output.artifacts` → sign → `inject-files` model is also what a
> dedicated external IC-native signer would implement — signing the UKI/shim/bootloader **and the
> dm-verity root hash**, with an **ephemeral** *or* a **remote/production** key source. So the design
> below is sound; such a tool plugs in as one more **signer backend** for the host-side step (a
> superset of the openssl+sbsign backend). See `2026-07-22-signing-step1-ic-native.md`.
...
- Be **backend-pluggable**: a local test-CA (port of `sign.py`) for CI, with room for real signing
  services (Key Vault / PKCS#11 / remote signing service) without touching the orchestration.
...
1. **Customize → a raw intermediate.** A signed cell **forces scratch staging** (the
   ...
   `meta/docs/2026-06-29-output-artifacts-staging.md` §3.2), and customize runs with
...
> rather than **signer identity** (`2026-06-29-signing-status.md`), so signed outputs are not yet cached on the
...
## 11. Milestones (refines 2026-06-22-design.md §17 M4)
