...
> **Update (2026-07-22):** this exact `output.artifacts` → sign → `inject-files` model is what
> **external-signer** implements as an IC-native signer — it signs the UKI/shim/bootloader **and the
> dm-verity root hash**, with **ephemeral** *or* **remote-service** (production) methods. So the design below is
> sound; the concrete recommendation is to make external-signer a **signer backend** for the host-side step
> (a superset of the openssl+sbsign backend). See `2026-07-22-signing-step1-ic-native.md`.
