...
> **Status:** _implemented (2026-09-09)._ `base: { image }`, the `inputs:` embed (`${inputs.<name>}`),
> `dependsOn`, the image DAG (topological ordering, cycle detection), and per-node plan/build all
> shipped (`crates/tailor-core/src/imagedep.rs`; reference: `docs/reference/image-yaml.md`). Motivated
> by two recurring shapes that previously forced either a hand-written cross-image path (fragile) or a
> **nested workspace** built by two ordered `tailor build` invocations (a workaround). This doc
> records the design: a first-class dependency edge between workspace images, its config surface, and —
> the crux — how it stays **incrementally correct** under tailor's plan-then-build architecture. Open
> refinements in §9.
